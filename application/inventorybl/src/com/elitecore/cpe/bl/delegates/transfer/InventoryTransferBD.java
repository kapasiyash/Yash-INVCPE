...
private static final String MODULE ="TRANSFER-INVENTORY-BD";
	...
	public void acceptRejectedTransferInventory(
			PartialAcceptRejectTransferOrderVO transferOrderVO) throws CreateBLException{
		try {
			getFacade().acceptRejectedTransferInventory(transferOrderVO,getBLSession());
		} catch (NamingException e) {
			e.printStackTrace();
	...
	public InventoryVO checkInventoryForAcceptRejectedInventory(String inventoryNumber,
		...
		try {
			return getFacade().checkInventoryForAcceptRejectedInventory(inventoryNumber,orderNumber);
		}catch (NamingException e) {
			e.printStackTrace();
			return null;
		}catch (SearchBLException e) {
